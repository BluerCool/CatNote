...
namespace StyleSheet
{
	...
	inline QString toolBtnQss(
		bool isCloseButton = false,
		const QColor& defaultColor = QColor("#BFBFBF"), 
		const QColor& hoverColor = QColor("#1677FF"))  // 新增参数：是否为关闭按钮
	{
		if (isCloseButton) {
			// 关闭按钮特殊样式（红色悬停）
			return QString(R"(
	...
	inline QString closeButtonQss()
	{
		return 
			...
			"}"
			"QToolButton:hover {"
			"   background-color: rgba(255, 0, 0, 0.8);"  // 红色半透明
			"}"
			"QToolButton:pressed {"
			"   background-color: rgba(200, 0, 0, 0.9);"  // 深红色
			"}";
	}

}

inline QString themeSwitcherBtnQss()
{
