...
DesignSystem::DesignSystem(QString videoName, QString videoPath)
	: m_mode(Light)
{
	loadThemes();
	...
	if (!QFile::exists(homeVideoPath))
	{
		...
		homeVideoPath = QCoreApplication::applicationDirPath() + "/Video/" + videoName;
	}
}
...
{
	// Light Theme
	m_lightTheme.primaryColor = QColor(22, 119, 255);	// 蓝色
...
void DesignSystem::setAnimationDuration(int ms)
{
    if (m_animationDuration != ms) {
        m_animationDuration = ms;
        emit animationDurationChanged(ms);
    }
}

int DesignSystem::animationDuration() const
{
    return m_animationDuration;
}

void DesignSystem::saveThemeToJson()
{
    QString configPath = QCoreApplication::applicationDirPath() + "/settings.json";
    ...
    obj["themeMode"] = (m_mode == Dark) ? "dark" : "light";
...
void DesignSystem::loadThemeFromJson()
{
    QString configPath = QCoreApplication::applicationDirPath() + "/settings.json";
    QFile file(configPath);
...
QString& DesignSystem::newFileIconPath()
{
    if (m_mode == Light) {
        newFileIcon = ":/Imgs/newfile.svg";
    } else {
        newFileIcon = ":/Imgs/newfileDark.svg";
    }
    return newFileIcon;
}

QString& DesignSystem::openFileIconPath()
{
    if (m_mode == Light) {
        openFileIcon = ":/Imgs/openfile.svg";
    } else {
